...
if python -m py_compile \
  ...
  wordmap_gpt2.py event_guidance.py dialogue_session.py priming.py associative_cascade.py cognition_cache_guard.py dialogue_corpus.py \
  ...
  && python -c "import launch, credit_learning; assert 'wuWorkspace' in launch.wordmap_mobile.HTML; assert 'wuDeleteHistoryBtn' in launch.wordmap_mobile.HTML; assert 'wuCognitionPanel' in launch.wordmap_mobile.HTML; assert launch.core.priming_version == '0.18.0'; assert launch.core.associative_cascade_version == '0.18.0'; assert launch.core.cognition_cache_guard_version == '0.18.0'; assert credit_learning._origin_key('점화') == '점화'; assert credit_learning._origin_key('연상 폭포') == '연상 폭포'; print('[WordMap] v0.18 runtime wiring: OK')"; then
  ...
  echo "[WordMap] 질문마다 휘발성 연상 캐시를 비워 오래된 그래프 상태가 다음 질문에 섞이지 않습니다."
